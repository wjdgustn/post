...
app.post('/join', utils.isNotLogin, async (req, res, next) => {
    ...
    if(checkNick != null) {
        req.flash('joinError', '이미 해당 닉네임이 사용 중입니다.');
        return res.redirect('/join');
    }
    if(req.body.nickname == '' || req.body.email == '' || req.body.password == '') {
        req.flash('joinError', '필수 입력란을 입력하지 않았습니다.');
        return res.redirect('/join');
    }
...
app.get('/find_my_password', (req, res, next) => {
    res.render('find_my_password', {
        ...
        logined: req.isAuthenticated(),
        isAdmin: req.isAuthenticated() && setting.ADMIN.indexOf(req.user.snsID) != -1,
        FindMyPasswordError: req.flash('FindMyPasswordError'),
        FindMyPasswordInfo: req.flash('FindMyPasswordInfo'),
        servername: setting.SERVER_NAME
    });
});
...
app.post('/find_my_password', async (req, res, next) => {
    ...
    if(user == null) {
        req.flash('FindMyPasswordError', '존재하지 않는 계정입니다.');
...
`
    ...
    req.flash('FindMyPasswordInfo', '이메일을 발송했습니다. 이메일로 보내진 링크를 눌러 비밀번호를 변경하세요.(30분간만 유효합니다.)');
